...
void loop() {
  ...
  {
      // Output a single, constant HIGH/ON 5V signal until changed to OFF/LOW
      digitalWrite(digitalOutput1, HIGH); // pin 2

      // Outputs a PWM wave that is approximately 50% duty cycle until changed to OFF/LOW
      ...
      PORTB = B00000011; // Sets pins 8 and 9 as high simultaneously (non-PWM, i.e. similar to digitalWrite but instantly) until changed to OFF/LOW
  }
  
  // Delay to allow for another button press to register if desired (i.e. you can hold it for 1 second but it will only execute the above commands once, then after 1 second it will execute the loop again), value is in miliseconds
  delay(1000);
}
